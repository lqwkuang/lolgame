Shader "Unlit/Fogshader"
{
    Properties
    {
        ...
        _BaseTerrainTex ("Base Terrain", 2D) = "white"{} // 基础地形纹理
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            sampler2D _MainTex;
            fixed4 _Color;
            sampler2D _BaseTerrainTex;      // 基础地形纹理
            sampler2D _MaskTex;
            ...
            fixed4 frag (v2f i) : SV_Target
            {
                ...
                fixed4 terrainColor = tex2D(_BaseTerrainTex, i.uv); // 基础地形采样
                fixed mask = tex2D(_MaskTex, float2(uv.x, uv.y)).r; //return mask;
                return lerp(terrainColor,col , mask);
            }
            ENDCG
        }
    }
